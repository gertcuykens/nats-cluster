...
func usage() {
	log.Fatalf("Usage: stan-bench [-s server (%s)] [-tls path] [-id CLIENT_ID] [-np NUM_PUBLISHERS] [-ns NUM_SUBSCRIBERS] [-n NUM_MSGS] [-ms MESSAGE_SIZE] [-csv csvfile] [-mpa MAX_NUMBER_OF_PUBLISHED_ACKS_INFLIGHT] [-io] [-a] <subject>\n", nats.DefaultURL)
}
...
func main() {
	...
	var tls = flag.String("tls", "/etc/tls", "TLS certificates")
	...
	path := *tls
	cert := nats.ClientCert(path+"/nats.pem", path+"/nats-key.pem")
	root := nats.RootCAs(path + "/ca.pem")
	user := nats.UserInfo("nats_client_user", "nats_client_pwd")
	...
	log.Println("servers:", opts.Servers)
	opts.Secure = true
	cert(&opts)
	root(&opts)
	user(&opts)
...
func runPublisher(startwg, donewg *sync.WaitGroup, opts nats.Options, numMsgs int, msgSize int, async bool, pubID string, maxPubAcksInflight int) {
	...
	snc, err := stan.Connect("nats-streaming", pubID, stan.MaxPubAcksInflight(maxPubAcksInflight), stan.NatsConn(nc))
	if err != nil {
		log.Fatalf("Publisher %s can't connect: %v\n", pubID, err)
	}
...
func runSubscriber(startwg, donewg *sync.WaitGroup, opts nats.Options, numMsgs int, msgSize int, ignoreOld bool, subID string) {
	...
	snc, err := stan.Connect("nats-streaming", subID, stan.NatsConn(nc))
	if err != nil {
		log.Fatalf("Subscriber %s can't connect: %v\n", subID, err)
	}
