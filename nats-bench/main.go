...
func usage() {
	log.Fatalf("Usage: nats-bench [-s server (%s)] [--tls path] [-np NUM_PUBLISHERS] [-ns NUM_SUBSCRIBERS] [-n NUM_MSGS] [-ms MESSAGE_SIZE] [-csv csvfile] <subject>\n", nats.DefaultURL)
}
...
func main() {
	...
	opts := []nats.Option{cert, root, user}
...
func runPublisher(startwg, donewg *sync.WaitGroup, opts []nats.Option, numMsgs int, msgSize int) {
...
func runSubscriber(startwg, donewg *sync.WaitGroup, opts []nats.Option, numMsgs int, msgSize int) {
	nc, err := nats.Connect("nats://nats:4222", opts...)
	if err != nil {
		log.Fatalf("Can't connect: %v\n", err)
	}
